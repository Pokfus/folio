...
# The L1–L10 batch log, moved out of `CLAUDE.md` (2026-09-11)

**READ BEFORE OPENING A BATCH THAT SHORTENS CITED PROSE.** The per-batch account of the length pass as
it stood in `CLAUDE.md` until it was moved here verbatim: the padding class each kind of term turned out
to have, the sibling-duplication lever and where it stopped working, the splitter gaps found on the way,
and every blind spot in the figure, marker and hedge checks. The RULES it produced stay in `CLAUDE.md`.

**L3 recast the "keep it where the borders ARE the
...
"8,000 **calendar** years ago" reads as lost when it survived.
