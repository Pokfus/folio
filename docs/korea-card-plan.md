...
**READ BEFORE WRITING A `ko-` CARD.** The account of `ko-001`–`ko-100` as it stood in `CLAUDE.md` until
it was moved here verbatim: what carried each batch of ten, which hosts answer and which are shut, the
four distinct routes to a wrong sort year, and the content decisions worth re-reading rather than
re-making. The RULES it produced stay in `CLAUDE.md`.

**Its first ten are geography rather than events, and that shaped the
