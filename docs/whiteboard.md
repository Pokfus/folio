...
# The marker's rules in full, moved out of `CLAUDE.md` (2026-09-11)

**READ BEFORE CHANGING THE MARKER'S GESTURES OR ITS PANEL.** The account as it stood in `CLAUDE.md`
until it was moved here verbatim: the bug reports behind the gesture-ownership rules and exactly how a
palm produced each reported symptom, the map-window drag reproduced through CDP touch input, and the
coach marks that were covering the control they described. The RULES stay in `CLAUDE.md`.
