...
**THE PASS IS COMPLETE AS MEASURED ON 2026-09-06: all 1,458 cards in the History and Science sections
carry three authored why-questions with their answers.** It closed twice, which is the lesson worth
keeping: it first ran to the end of its own list at 1,159 cards, and then 300 more — 200 in Ancient Rome,
100 in the Second World War — turned out to have landed on main while it ran, so the sections were not
finished though the list was. **A pass measured against a corpus that is still growing cannot close.**
What keeps it closed is the rule rather than the pass — a new History or Science card ships with its own
set, exactly as it ships with its citations and its glossary term — and any claim of completeness here
names the day it was measured. **Run `node .claude/why-count.js` rather than trusting a figure.**
...
| 2026-09-06 | `rm-101`–`rm-300`, `ww2-001`–`ww2-100` (300 cards) | **THE SECTIONS ARE COMPLETE: 1,458 of 1,458**, closing the gap the merge exposed. Ancient Rome's second and third hundreds keep the collection's own seam — the claim resting on nothing measurable — and add the annalistic one: a battle reported by two authors with irreconcilable numbers, a law whose text is lost and known only from what a later writer says it did. The Second World War is the collection this file had no experience of, and it offers a seam none of the others do: **the legal machinery of a state used to destroy that state, lawfully**. An emergency decree suspends the constitutional protections rather than repealing them; an enabling act is passed by a parliament voting itself out of existence; a clause already in the criminal code since 1871 is rewritten wider and convictions multiply; a citizenship law defines nobody and needs a regulation two months later to say who a Jew is, which sends a racial statute to church and synagogue records for its answer. **Ask what the measure did not have to change**, and the answer is usually that the instrument was already there. Two more that recur. **A figure the card gives with its source named writes its own question** — the tribunal's 200,000 at Nanjing against burial societies' 155,000 counted bodies, T4's own reckoning of 70,273 — and the honest answer says who counted and how, never which figure is right. And on the Pacific decks the seam is constitutional: an army answering to the emperor rather than the cabinet, a rule requiring serving officers as ministers, and soldiers in the field presenting their government with accomplished facts, which makes **why could the government not stop this** the question the collection keeps supplying. |
| 2026-09-06 | `us-001`–`us-100` (100 cards) | **THE UNITED STATES IS COMPLETE**, and it is the first collection written under the new rule rather than against a list: `add-card.js` now REQUIRES a `why` on a new card, so this backlog is a backlog rather than a moving target. Its Native America deck offers a seam the history collections had not met, and it is the strongest in the file so far — **the evidence that had to be found somewhere other than where the question was asked**. Human remains cannot be sampled freely, so turkeys and dogs carry the migration; a village chronology built on European trade goods was dating the arrival of the goods, so 86 radiocarbon measurements moved it decades; a population collapse is counted in an ice core because 55.8 million hectares went out of cultivation; a chewed wad of yucca answers a question about ancestry. **Ask what stands in for the evidence nobody can take**, and the answer is usually stated in the card's own prose. Two others recur. **A colonial document is a source about its author** — the traders' credit was extended past what could be repaid in order to force land concessions, the Proclamation line was fiscal as well as protective, the praying town sold a religion and a way of living together — so the third question is repeatedly what the record was FOR. And **a legal card answers from the gap between the ruling and the ground**: `Worcester v. Georgia` is a complete victory that changed nothing, which is the sharpest answer in the collection and belongs to the reader rather than to the law. |
| 2026-09-06 | `jp-001`–`jp-100` (100 cards) | **JAPAN IS COMPLETE**, and it is the collection where the evidence seam is at its purest, because for its first eight decks there is no written source at all: ten thousand years of prehistory read off ash, pots, bones and the ground they lie in. So the questions are about instruments. **Ask what stands in for a document** — a bed of volcanic ash dating a layer countrywide, an obsidian trace element naming an outcrop, a fat soaked into a pot saying what was cooked in it, a seed impression in fired clay that swelled before it burned away. Two findings specific to this collection. **A CARD ABOUT A FRAUD IS THE STRONGEST WHY-CARD ON THE SHELF**: the planted Paleolithic ran for a quarter of a century, and the four conditions written afterwards are a discipline stating how it will refuse the next one, which is a reason a reader can check. And **where the record IS written, it is written abroad** — the Chinese histories are Japan's only text for six centuries — so the third question is repeatedly what a foreign court had reason to record, why its sailing directions place a queen's country in the open sea, and why a Wa king's claim to command Baekje was struck off the list every time he made it. From the Asuka period onward the collection turns institutional and the seam changes with it: a code known only through later commentaries, a chronicle compiled by the state it describes, a title written back into reigns that never used it. |
