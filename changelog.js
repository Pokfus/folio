...
window.FOLIO_VERSION = { v: "1.150", released: "2026-08-10T08:15Z" };

window.CHANGELOG = [
  {
    d: "2026-08-10",
    t: "You can gather your decks into groups, the Daily study banner changes colour every day, both of Homer's poems join the Library with the Greek facing them, and the daily games stop asking about words nobody has heard of",
    items: [
      ...
      "<b>Both of Homer's poems are on the shelf</b> — the Iliad and the Odyssey, twenty-four books each, in A. T. Murray's Loeb translations of 1924 and 1919, with the Greek facing them. The two columns are paired on Homer's own line numbers, so an English passage and the Greek beside it are the same place in the poem, and on a phone you can tap the page to turn it over from one to the other. Murray translates into prose rather than verse, which is a first for the Library: every book here that carries its original has had verse facing verse or prose facing prose. His printed pages also carry footnotes, and the transcription Folio works from kept only the numbers that pointed at them and none of their text, so both editions ship without them. Each book's own front matter says so, and says which lines its Greek leaves out — eight in the Iliad, three in the Odyssey.",
