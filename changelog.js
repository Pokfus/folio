...
window.FOLIO_VERSION = { v: "1.147", released: "2026-08-10T06:20Z" };

window.CHANGELOG = [
  {
    d: "2026-08-10",
    t: "One note can now make several cards, so a word can be asked in both directions — plus a second scheduler you can choose deck by deck, a note's other cards stay out of your way until tomorrow, every review is written down, and a card can tell you why it is due",
    items: [
      "<b>You can now choose how a deck is scheduled.</b> Beside the intervals Folio has always used there is now <b>FSRS</b>, which models how a memory itself fades — how firmly a card is held and how hard you find it — rather than multiplying the last interval by a number. Hold a deck's row in your daily study and open <b>Scheduling</b> to switch, deck by deck: a vocabulary deck can be on one and a history deck on the other, and the pooled review schedules each card the way its own deck asks. You also say what you are aiming for — how much you want to still remember when a card comes back, 70% to 98% — and Folio spaces it to hit that. Turning it on costs you nothing you have already learned: a card you have been studying for months carries its current interval across as its starting strength, so nothing is reset and nothing is re-taught. If you have had your own parameters worked out in Anki you can paste those in too.",
      "<b>A note's other cards now stay out of your way until tomorrow.</b> Answering 中 → middle and then middle → 中 an hour later tests the last hour rather than the word, so answering one card of a note puts its siblings off until the next day — which is what makes asking a word in both directions worth doing. It is on by default and can be turned off for any deck that has notes with more than one card. Nothing is lost: a card put off is due tomorrow, not skipped, and taking a grade back brings its siblings back with it.",
  ...
  },
  {
    ...
    items: [
      "<b>Your own decks can have subdecks.</b> A deck you write or import can group its cards — a language deck with a direction each way, a course with a chapter each — and each group is added to your daily study and studied on its own, the way a collection\u2019s decks are. They show as indented rows under the deck on the Collections page, each with its own count, progress and + button, and a deck with no groups looks exactly as it always did. A subdeck travels inside the deck file, so one file now holds what used to be two.",
      "<b>A deck can read its answers aloud by itself.</b> Hold a deck on the home page and turn on “Read aloud automatically”, and from then on revealing a card speaks it — no button to press. The switch appears only where there is something to hear, which today means a deck of your own whose card type marks text to be read aloud, and it starts off: a site that is going to make a noise should be asked to first. The speaker button on the card itself is unchanged for anyone who would rather press it.",
      "<b>Folio has pictures.</b> 360 cards, 771 glossary terms and 99 of the 100 artefacts in your collection now carry an illustration — a handaxe, a fresco, a portrait, the Rosetta Stone, a satellite view of Mesopotamia — where before there was exactly one picture on the whole site. Every one comes from Wikimedia Commons at high resolution and is free to use commercially: most are public domain, and the rest are Creative Commons pictures whose photographer is named in the caption, as their licence requires. Each carries its title, a description and a link to the file it came from, so you can always see where it is from and check it yourself. Click any picture to open it full size. Countries are illustrated by their flag, and where nothing suitable could be found the card simply has no picture rather than a doubtful one — which is most of what is left: the terms still without one are largely ideas rather than things, like the Spartan council or the Greek word for a clan, and living scholars whose photographs are not free to reuse. From now on a new card, glossary term or artefact looks for its picture as it is written, so the pictures keep up with the writing instead of arriving in an occasional sweep.",
      "<b>A card's picture now sits at the top right of its background</b>, with the prose running down beside it — the way a glossary term's picture has always sat in its popup. It used to take a band of its own above the first line, which pushed the writing a picture's height down the card. The frame is also the shape of whatever is in it: pictures are shown whole rather than cropped to a letterbox, and a portrait or a standing figure no longer sits between two panels of grey paper.",
      ...
      "<b>Three new daily games</b>, taking the home page's grid to nine. <b>Crossword</b> builds a fresh grid every morning out of the cards' own answer terms, and every clue is a card's own question with the answer taken out of it — so it is the deck read sideways, answered a letter at a time. An answer turns green the moment you have it right and red when it is wrong, and a wrong one can simply be typed over — nothing is ever revealed, so the grid stays open all day and you can leave it half done and come back. The only way to lose is to reach the end of the day with an answer still missing. <b>What year?</b> lists five things that all belong to one year and gives you a timeline and three guesses to find it; guess too early and the rail tells you so and rules out everything below. And a <b>Picture round</b> asks you to name what is in five illustrations, drawn from every picture on the site — a card's, a glossary term's or an artefact's.",
      "Winning every daily game in one day still earns the Clean Sweep badge and a chest — and now means all nine of them.",
      "<b>Every badge you unlock now earns a chest</b>, one for each, on top of the ones a new level and a clean sweep already bring. They wait in your account rather than opening on the spot, so a badge won mid-card no longer interrupts the card.",
      "Three more of the daily quotes can be turned over to read in the language they were written in: Ibn al-Haytham's and al-Ghazali's Arabic, and Ibn Khaldun's. Two others were looked at and deliberately left in English — Ashoka's edict, because no readable copy of the inscription's own spelling could be found, and the line under Maimonides' name, which turns out to be older than he is.",
      ...
      "Each book on the Library shelf now says whether it carries the text in the language it was written in, without your having to open it: a small chip on the banner naming the original — Greek, Latin, Old English, Old French, Sanskrit, Italian, Chinese — on the twenty of the thirty books that have one. Four books that do carry theirs used to call it simply “the original” inside; they name their language now too.",
      "The books on the Library shelf are properly coloured again, in daylight and after dark. Each book's colour was going grey at both ends — too dark to tell apart on a white page, and washed out almost to nothing on a black one — because lightening a colour by adding white takes the colour out of it. Every spine, author line and wash now keeps its own hue at full strength, and reads more clearly against the page than before. The author's name above each title is also larger, bolder and a shade deeper, so it can be read at a glance on every book — it was the smallest type on the banner, and on a few books it was too faint to make out at all.",
      ...
      "Any single deck can now be set to shuffle, or not to, by holding its row under Daily study — the choice used to be available only for the whole day's study at once. Settings still sets what a deck does until you say otherwise.",
      "Ancient Rome wears a laurel wreath instead of a triumphal arch, the Second World War collection is now called World War II, the What year? game tile has traded its mustard for a leaf green, and in the editor the Quotes and Artefacts tabs have colours of their own like the other five. In high contrast the crossword's clue numbers and enumerations are readable on the clue you have selected, which they were not.",
  ...
  },
  {
    ...
    items: [
      ...
      "The first thing the home page asks you to do now takes you to the collections, so the subject you start with is yours to pick rather than one Folio picks for you. Once you have added one, the button on the home page deals you a card as it always did.",
      ...
      "The Library now explains itself the first time you open it, as the Atlas already did — and it does so in two halves, each where it is any use. The shelf tells you what may be shelved and why, and how to search and sort it; the reading — the chapters, the original language beside the translation, the marker and the highlights — is explained the first time you actually open a book, beside the bar it is talking about. The Atlas card has gained a line about the marker too, since the strokes you draw on the globe are pinned to the map and turn with it. All three can be reopened from the ? beside them, whenever you like.",
      "The home page says less to a first-time reader and means more by it. The three tiles under the daily study banner explaining what a flashcard is are gone — the walkthrough above them covers all of it properly, with a real card to look at — and the opening banner now offers one way in rather than two. On a wide screen the pages in the top bar are named in full at all times, instead of unfolding their names only when pointed at, and About Folio at the foot of the page stands well clear of the games above it.",
      "The daily quote can be selected and copied again. Clicking it still turns it over into its original language; sweeping across the words no longer does.",
      "<b>Reset progress now resets your progress, and nothing else.</b> It was clearing the whole of your saved Folio — your theme, your text size, your language, your sound and reading settings, where the Atlas opens, and the decks you had added to Daily study — none of which the button ever said it would touch. It now clears what it names: your card history, your streak, your badges and your artefacts. Your decks and their daily limits, your place in each Library book, the books you have starred and every setting are kept, and the confirmation says so before you type RESET.",
      "…and resetting no longer turns the home page back into a first-time visitor's. If you have decks in your Daily study you now see them, and a Start button, rather than the opening screen with the list underneath it taken away.",
      "On dark themes the Library shelf reads properly again: the author above each title, the Start reading line and the coloured spine were set in colours chosen for a white page, and several of them all but vanished against the dark one. Each book keeps its own colour and wears a lighter shade of it after dark.",
