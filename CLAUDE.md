...
- `docs/world-geography-card-plan.md` — the running order for **World** (`geo-world`, the second
  ...
  **Israel and Palestine are deferred**, numbered but not written, because a card that shades a shape and
  ...
  · **📖 `docs/geography-background-plan.md` — READ BEFORE REWRITING A `gw-` BACKGROUND.** These
    ...
    **G1–G5 have shipped — sixty-one backgrounds rewritten** (`gw-001`–`gw-074`, less the deferrals
    below), each five sentences of landform, water and climate over five of the country's own
    history, with forty-seven date lines rewritten out of the same research. Three sources make the
    ...
    **The CBD's country profiles** carry a country's own ecosystems in prose, which no statistical
    ...
    PDF**, Afghanistan's being served as 1.6 MB of HTML, which is why `gw-036` is deferred. **ELEVEN
    HIGH-INCOME COUNTRIES HAVE NO AQUASTAT PROFILE AT ALL** (`gw-003`, `gw-019`, `gw-021`, `gw-023`,
    `gw-025`, `gw-032`, `gw-037`, `gw-042`, `gw-054`, `gw-067`, `gw-072`) and are deferred rather than
    half-solved: the
    ...
    an event rather than a state. Rules 1 and 3 remain open on about 360 cards.
