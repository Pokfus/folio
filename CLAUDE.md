...
- `books/<id>.js` — one **Library book**'s text: `window.FOLIO_BOOKS_IN.push({ id, intro, chapters:[{ n, p, t, html, notes }] })`.
  **Lazy** (bundle `book:<id>`), **generated — never hand-edited** (see `.claude/fetch-book.js`), and it pushes onto a
  ...
  matter (chapter 0 — see the Library bullet). Currently sixteen: `seneca-letters`,
  ...
  **not an integer**, being a Bekker page like `1094a`; see the `data-n` note in the Library bullet)
  ...
  only the number tells them apart, which is the sharpest argument for numbering them at all).
- `books/<id>.<lang>.js` — the same book in the language it was WRITTEN in
  ...
  Its own **lazy** bundle (`bookOrig:<id>`), generated by the same importer, never hand-edited. Currently fourteen:
  ...
  **Sixteen books, fourteen originals**: the Republic and Aesop's Fables have none, and the reason is the
...
- **THE LIBRARY — whole books, read on the site** (`PAGES.library` at `#library`, `PAGES.book` at
  ...
  · **WHAT MAY BE SHELVED, and it is the only content rule.** Folio serves the text itself, so a book goes
    ...
    **The Peloponnesian War is the THIRD needing no qualification at all** (Aug 2026), after the Republic
    ...
    not always the one worth reading**; the cost was one missing chapter number in 917. Each book's
