...
- `docs/china-card-plan.md` — the **1000-card running order for the China collection** (`china`): every
  ...
  see the "CHINA" bullet under "Generating cards & glossary entries". **`cnh-001` to `cnh-179` have
...
| China | `china` | `cnh-` | `docs/china-card-plan.md` | 7 / 39 | 179 cards, contiguous — next is `cnh-180`; the collection is open to study |
...
**`node .claude/test-card-plans.js` checks all of this** (248 assertions, no browser, no dependencies):
...
- **Forty-eight committed regression tests** (in `.claude/`, not loaded by the site — the count excludes
  ...
  **And close any IndexedDB connection the test itself opens** — an idle one blocks the app's own open after a
  ...
  · `node .claude/test-card-plans.js` — 248 assertions on **the join between the sixteen card plans and
