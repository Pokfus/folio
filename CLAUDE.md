...
- `docs/world-geography-card-plan.md` — the running order for **World** (`geo-world`, the second
  ...
  **Israel and Palestine are deferred**, numbered but not written, because a card that shades a shape and
  ...
  · **📖 `docs/geography-background-plan.md` — READ BEFORE REWRITING A `gw-` BACKGROUND.** These
    ...
    **G1–G8 have shipped — ninety-seven backgrounds rewritten** (`gw-001`–`gw-128`, less the
    deferrals below), each five sentences of landform, water and climate over five of the country's own
    history, with eighty-three date lines rewritten out of the same research. Three sources make the
    ...
    **The CBD's country profiles** carry a country's own ecosystems in prose, which no statistical
    ...
    PDF**, Afghanistan's being served as 1.6 MB of HTML, which is why `gw-036` is deferred. **TWENTY-NINE
    HIGH-INCOME COUNTRIES AND TERRITORIES HAVE NO AQUASTAT PROFILE AT ALL** (`gw-003`, `gw-019`, `gw-021`,
    ...
    `gw-115`, `gw-117`, `gw-119`, `gw-120`, `gw-121`, `gw-123`) and are
    ...
    empty there as well. Rules 1 and 3 remain open on about 330 cards.
