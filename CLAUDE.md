...
- `books/<id>.js` — one **Library book**'s text: `window.FOLIO_BOOKS_IN.push({ id, intro, chapters:[{ n, p, t, html, notes }] })`.
  **Lazy** (bundle `book:<id>`), **generated — never hand-edited** (see `.claude/fetch-book.js`), and it pushes onto a
  ...
  matter (chapter 0 — see the Library bullet). Currently three: `seneca-letters`,
  `marcus-aurelius-meditations` (~337 KB, all 12 books, 487 section numbers, 812 translator notes) and
...
- `books/<id>.<lang>.js` — the same book in the language it was WRITTEN in
  ...
  Its own **lazy** bundle (`bookOrig:<id>`), generated by the same importer, never hand-edited. Currently two:
  `seneca-letters.la.js` (~862 KB, all 124 letters) and `sun-tzu-art-of-war.zh.js` (~35 KB, all 13 chapters —
  classical Chinese is terse, and this is the whole work).
  **The Art of War is the case the Meditations note below said would settle it, arriving.** Its original is not
  another edition on another wiki: it is the Chinese Giles printed on the FACING HALF OF HIS OWN PAGE,
  transcribed in the same table, carrying his own section numbering explicitly (each printed page's list opens
  on an `<li value="N">`). So the numbers are stated by the edition rather than counted off a list, and they are
  the English side's numbers by construction. Measured before it was believed, over all 13 chapters: **385
  sections, the two columns agreeing exactly** — same count, same maximum, nothing on one side missing from the
  other, no duplicates. Hence no table of corrections and no hedging. **The lesson is that the blocker is never
  "is there an original?" but "does the original say which section each passage is?"** — and the surest yes is a
  facing-page edition, where one editor numbered both columns at once.
  And **the Meditations deliberately has none**, which is the
  case worth reading before adding an original: the licence is fine (Marcus wrote in Greek in the 170s) and what
  stops it is that **Greek Wikisource prints no section numbers** — each book is one `<ol>`, so the only handle is
  list POSITION, and position is not the same claim as a number. The two editions divide six of the twelve books
  differently (measured: pairing by position correlates 0.98–1.00 on the six books whose counts agree and
  0.33–0.71 on the six that diverge), so past one splice point per book the position runs one out from the section
  it would have to be. The offsets were NOT guessed at: a wrong pairing is not a missing feature but a lying one,
  since the reader is shown one passage beside another with nothing on the page to say so. **A book with no
  `origLang` simply shows no original-language control**, which is the graceful half of this. See the long comment
  at the foot of that book's entry in `.claude/fetch-book.js` for what would settle it.
...
- `.claude/fetch-book.js` — the importer that writes those files, from Wikisource. Standalone Node helper,
  ...
  **A book's ORIGINAL language is a second half of the same entry** (`original: { lang, langName, wiki, pages, … }`),
  written to `books/<id>.<lang>.js` and fetched from a DIFFERENT wiki, with its own cache under
  `book-cache/<id>/<lang>/`. Two things about that half are worth knowing before adding one.
...
- **THE LIBRARY — whole books, read on the site** (`PAGES.library` at `#library`, `PAGES.book` at
  ...
  · **WHAT MAY BE SHELVED, and it is the only content rule.** Folio serves the text itself, so a book goes
    ...
    other two use**. (The Chinese underneath is ~25 centuries old and free everywhere.) Each book's
  ...
  · **THE ORIGINAL BESIDE THE TRANSLATION** (`bookSections` / `bookRows` / `applyLangMode` /
    ...
    · **PAIRED ON SECTION NUMBERS, NEVER ON PARAGRAPH ORDER**, and the obvious implementation is the
      ...
      unpaired (0.7%), in 7 letters**, and those are gaps in the Latin edition's own numbering.
