...
def headword_html(e, gender):
    ...
    for half in e['display'].split(', '):
        m = re.match(r'^(der|die|das|der/die)\s+(.*)$', half)
        if m:
            g = gender if half == e['display'].split(', ')[0] else ''
            cls = GENDER_CLASS.get(ARTICLE_G.get(m.group(1), g), '')
            parts.append(f'<span class="uc-art {cls}">{esc(m.group(1))}</span> {esc(m.group(2))}')
        else:
...
for i, e in enumerate(entries, 1):
    ...
    if not gender and e['article'] in ARTICLE_G:
        gender = ARTICLE_G[e['article']]
    ...
    if pos == 'noun' and gender:
        label += ', ' + {'m': 'masculine', 'f': 'feminine', 'n': 'neuter'}.get(gender, gender)
    if e['pluralonly']:
        label += ', plural'
    if e['reflexive']:
    ...
    conj = conjugation_html(rec, e['reflexive']) if pos == 'verb' else ''
    forms = forms_html(e, rec, pos)
    exs = EX.get(e['key'], [])
    stats['verbs with a paradigm'] += bool(conj)
