/* Folio — the profile photo: choosing its crop, and enlarging someone else's (Aug 2026, on request)
   ...
   · **THE SAVED SQUARE IS RE-RENDERED FROM THE ORIGINAL**, not scaled out of the preview, so its size is
     `PHOTO_PX` whatever the screen was.
   ...
   Run:  NODE_PATH=<playwright>/node_modules node .claude/test-photo.js
...
const HOOK = "\n  window.__folioCrop = openPhotoCropper;\n  window.__folioPhotoView = openPhotoViewer;\n";
...
(async () => {
  ...
  const up = await page.evaluate(() => ({
    overlay: !!document.querySelector(".ph-crop"),
    canvas: !!document.querySelector(".phc-canvas"),
    ring: !!document.querySelector(".phc-ring"),
    zoom: !!document.querySelector(".phc-range"),
    square: (() => { const c = document.querySelector(".phc-canvas"); const r = c && c.getBoundingClientRect(); return r ? Math.abs(r.width - r.height) < 2 : false; })(),
    ringInert: (() => { const r = document.querySelector(".phc-ring"); return r ? getComputedStyle(r).pointerEvents === "none" : false; })(),
  }));
  ...
  const sample = () => page.evaluate(() => {
    const c = document.querySelector(".phc-canvas"), g = c.getContext("2d");
  ...
  const cb = await page.locator(".phc-canvas").boundingBox();
  ...
  const saved = await page.evaluate(() => {
    ...
    return new Promise((resolve) => {
      ...
      im.onload = () => {
        ...
        resolve({ w: im.width, h: im.height, jpeg: /^data:image\/jpeg/.test(uri), kb: Math.round(uri.length / 1024), left: px(0.12, 0.5), closed: !document.querySelector(".ph-crop") });
      };
  ...
  await page.waitForTimeout(500);
  await page.click(".phc-cancel");
  await page.waitForTimeout(300);
  const cancelled = await page.evaluate(() => ({ r: window.__result2, gone: !document.querySelector(".ph-crop") }));
  ...
  await page.evaluate((src) => window.__folioPhotoView(src, "Ada"), tiny);
  await page.waitForTimeout(400);
  const viewer = await page.evaluate(() => {
    const im = document.querySelector(".img-viewer .iv-img");
    if (!im) return null;
    const cs = getComputedStyle(im);
    return { open: true, avatarClass: im.classList.contains("iv-photo"), round: /50%/.test(cs.borderRadius), w: Math.round(im.getBoundingClientRect().width), alt: im.getAttribute("alt") };
  });
  ...
  await page.waitForTimeout(300);
  await page.evaluate(() => window.__folioPhotoView("", "Ada"));
