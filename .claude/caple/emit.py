...
if PHRASES:
    ...
    DESC = (
        ...
        "That covers what the dictionary marks, and an expression can be "
        "Brazilian in its WORDS while its entry is filed as Portuguese "
        "generally, which no marking can catch: a grama do vizinho é sempre "
        "mais verde is the grass being greener, and in Portugal a lawn is relva "
        f"and a grama is a gram. Every phrase here was swept against both "
        f"countries' word-frequency lists, and the {ST['hand']} whose wording "
        "gave them away were read and removed by hand. The sweep is a guide and "
        "not a rule, so a residue is left on purpose: it flags any phrase built "
        "from a word said far more often in Brazil, which catches em suma for "
        "the coincidence of suma and grão a grão enche a galinha o papo for "
        "papo, both of them Portugal's own. Removing everything it points at "
        "would cost more good phrases than bad ones. "
...
# shelf it often did not (Portuguese among them). The labelling is a pass over the FINISHED deck for
...
_dgst = _dg.dedupe(deck, 'Portuguese')
if _dgst['groups']:
