...
(async () => {
  ...
  /* ================= 2. the collection banners ================= */
  {
    ...
    // the figures are grouped (toLocaleString), so the separator is whatever the reader's locale uses
    check("…and counting cards, not XP", d.counts.every((x) => /^[\d,.\s]+ \/ [\d,.\s]+ cards$/.test(x)), JSON.stringify(d.counts));
