/* Two more things per word: a CHARACTER BREAKDOWN with radicals, and EXAMPLE SENTENCES.
   ...
   which is the syllabus's own reading rather than a guess.
     ...
     node extras2.js [levels…] → adds `chars` and `examples` to words1.json / words2.json                */
const fs = require("fs");
...
const ALL = [1, 2];
const FILE = (L) => ({ 1: "words1.json", 2: "words2.json" })[L];
...
function search(word, list, index, known) {
  const seed = index.get(word[0]) || [];
  const hits = [];
  for (const i of seed) {
    const s = list[i];
    if (s.t.indexOf(word) < 0) continue;
    if (!wordIsToken(s.t, word)) continue;   // 可爱 is not an example of 爱
...
function examplesFor(word, known) {
  const got = search(word, pool, byChar, known);
  return got.length ? got : search(word, wide, byCharWide, known);
}
